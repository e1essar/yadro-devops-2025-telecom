...
## Описание
- Определяем целевую систему как localhost (в inventory)
- Скачиваем официальный установочный скрипт Docker и выполняем его
- После установки проверяем установлен ли докер через docker --version
- Добавляем текущего пользователя в группу docker
- Запускаем и включаем сервис Docker
- Собираем docker-образ со скриптом
- Запускаем контейнер
- Проверяем что контейнер завершился успешно (!=0)
- Получаем логи контейнера через docker logs и выводим в консоль

## Тестирование
Запускаем плейбук командой (вводим become password после запуска команды):
```bash
